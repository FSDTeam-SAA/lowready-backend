...
export const referralSaving = catchAsync(
  async (req: Request, res: Response) => {
    ...
    // 1️⃣ Get facility IDs owned by this org
    const facilities = await Facility.find({ userId }).select('_id')
    const facilityIds = facilities.map((f) => f._id)

    // 2️⃣ Calculate Standard Referral Fee = sum of paid BookHome totalPrice
    ...
    // 3️⃣ Calculate Subscription Cost = sum of paid payments for this org
    ...
    // 4️⃣ Referral Fee (18% of standardReferralFee)
    const referralFees = (standardReferralFee * 18) / 100

    // 5️⃣ Total with your model
